# -*- encoding: utf-8
from __future__ import unicode_literals
from datetime import datetime

from django.db import models

# Create your models here.


class CityDict(models.Model):
	name = models.CharField(max_length=50, verbose_name=u"城市名")
	desc = models.CharField(max_length=50, verbose_name=u"描述")
	add_time = models.DateTimeField(default=datetime.now)

	class Meta:
		verbose_name = u"城市"
		verbose_name_plural = verbose_name

	def __unicode__(self):
		return self.name

class School(models.Model):
	CTG_CHOICES = (
		('bqx',u'百强校'),
		('szd',u'省重点'),
		('fdjg',u'辅导机构'),
	)
	name = models.CharField(max_length=50, verbose_name=u"学校名称")
	desc = models.TextField(verbose_name=u"学校描述")
	category = models.CharField(max_length=25, choices=CTG_CHOICES, verbose_name=u'类别', default='bqx')
	click_nums = models.IntegerField(default=0, verbose_name=u"点击数")
	fav_nums = models.IntegerField(default=0, verbose_name=u"收藏数")
	image = models.ImageField(upload_to="org/%Y/%m", verbose_name=u"学校Logo")
	address = models.CharField(max_length=150, verbose_name=u"学校地址")
	city = models.ForeignKey(CityDict, verbose_name=u"所在城市")
	student_nums = models.IntegerField(default=0, verbose_name=u"学习人数")
	course_nums = models.IntegerField(default=0, verbose_name=u"课程数")
	add_time = models.DateTimeField(default=datetime.now)

	class Meta:
		verbose_name = u"明星学校"
		verbose_name_plural = verbose_name

	def __unicode__(self):
		return self.name

class Teacher(models.Model):
	org = models.ForeignKey(School, verbose_name=u"所属学校")
	name = models.CharField(max_length=50, verbose_name=u"教师名")
	teach_years = models.IntegerField(default=0, verbose_name=u"教学年限")
	teach_subject = models.CharField(max_length=50, verbose_name=u"科目")
	points = models.CharField(max_length=50, verbose_name=u"教学特点")
	click_nums = models.IntegerField(default=0, verbose_name=u"点击数")
	fav_nums = models.IntegerField(default=0, verbose_name=u"收藏数")
	image = models.ImageField(upload_to="teacher/%Y/%m", verbose_name=u"头像", max_length=100, default='')
	add_time = models.DateTimeField(default=datetime.now)

	class Meta:
		verbose_name = u"授课教师"
		verbose_name_plural = verbose_name

	def __unicode__(self):
		return self.name
